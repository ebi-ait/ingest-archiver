...
class BioSamplesSubmitter:
    def __init__(self, aap_client: AapClient, biosamples_url: str, domain: str):
        self.__biosamples = BioSamples(aap_client, biosamples_url)
        self.__converter = BioSamplesConverter(domain)

    def send_all_samples(self, submission: Submission):
        ...
        for sample in submission.get_entities('biomaterials'):
            self.send_sample(sample, project_release_date)

    def send_sample(self, sample: Entity, release_date: str = None):
